...
ssize_t loopread(BZFILE* d,int*bzerror,u_char *buf,size_t nbytes)
{
	...
	for(ptr=0;ptr< ((ssize_t) nbytes);ptr+=lenread) {
...
int bsdiff_patch_file(char *patchFile, char *oldFile, char *newFile)
{
	...
	*/
	...
	if(fread(header,1,32,fd)!=32) errx(1,"Error reading %s",patchFile);
	...
	ctrlpipe=bz2read(&ctrlbz, &ctrlerror, 32,bzctrllen,patchFile);
	diffpipe=bz2read(&diffbz, &differror, 32+bzctrllen,bzdatalen,patchFile);
	if(version==2) {
		extrapipe=bz2read(&extrabz, &extraerror, 32+bzctrllen+bzdatalen,
			patchsize-(32+bzctrllen+bzdatalen),patchFile);
	};

	if(fclose(fd)==-1) errx(1,"Problem closing %s",patchFile);
