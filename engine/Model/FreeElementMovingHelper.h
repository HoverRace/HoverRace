...
class MR_ObstacleCollisionReport
{
	private:
		...
		void GetContactWithFeaturesAndActors(MR_Level * pLevel, const MR_ShapeInterface * pShape, int pRoom, MR_FreeElement * pElement, BOOL pIgnoreActors);
	...
	public:
		...
		MR_Int32 MR_DllDeclare StepHeight() const;// Negative Height of the floor if Not having contact, Shape height if higher than shape
												  // Negative Ceiling Height if Not having contact, Shape depth if lower than shape
		MR_Int32 MR_DllDeclare CeilingStepHeight() const;
												  // Negative if in floor
		MR_Int32 MR_DllDeclare SpaceToFloor() const;
												  // Negative if in ceiling
		...
		void MR_DllDeclare GetContactWithObstacles(MR_Level * pLevel, const MR_ShapeInterface * pShape, int pRoom, MR_FreeElement * pElement, BOOL pIgnoreActors = FALSE);
