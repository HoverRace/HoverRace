...
void CHoverCadView::OnDraw(CDC * pDC)
{
	...
	while(lPos != NULL) {
		...
		if(lPolySelected || lPolygon->mFeature) {
			...
			pDC->SelectObject(lPolygon->mFeature ? (lPolySelected ? &gSelectFeatureBrush : &gFeatureBrush)
		...
		for(lCounter = 0; lCounter < lNbAnchor; lCounter++) {
			...
			pDC->Pie(lAnchorXY[lCounter].x - HC_ANCHOR_RAY, lAnchorXY[lCounter].y + HC_ANCHOR_RAY, lAnchorXY[lCounter].x + HC_ANCHOR_RAY, lAnchorXY[lCounter].y - HC_ANCHOR_RAY, lAnchorXY[(lCounter - 1 + lNbAnchor) % lNbAnchor].x, lAnchorXY[(lCounter - 1 + lNbAnchor) % lNbAnchor].y, lAnchorXY[(lCounter + 1) % lNbAnchor].x, lAnchorXY[(lCounter + 1) % lNbAnchor].y);
	...
	while(lPos != NULL) {
		...
		pDC->Ellipse(lPoint.x - HC_NODE_RAY, lPoint.y + HC_NODE_RAY, lPoint.x + HC_NODE_RAY, lPoint.y - HC_NODE_RAY);
	}
