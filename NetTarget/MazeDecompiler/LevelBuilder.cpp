...
BOOL MR_LevelBuilder::Parse(FILE * pFile)
{
	...
	if(lReturnValue) {
		...
		for(lCounter = 0; lCounter < mNbRoom; lCounter++) {
			...
			mRoomList[lCounter].mWallLen = new MR_Int64[mRoomList[lCounter].mNbVertex];
		...
		for(lCounter = 0; lCounter < mNbFeature; lCounter++) {
			...
			mFeatureList[lCounter].mWallLen = new MR_Int64[mFeatureList[lCounter].mNbVertex];
...
double MR_LevelBuilder::ComputeShapeConst(Section * pSection)
{
	...
	lReturnValue = sqrt(pow(pSection->mMax.mX - pSection->mMin.mX, 2.0) + pow(pSection->mMax.mY - pSection->mMin.mY, 2.0)) / 1000.0;
