...
MR_MainCharacter::~MR_MainCharacter() { delete mRenderer; }
void MR_MainCharacter::SetAsMaster() { mMasterMode = TRUE; }
void MR_MainCharacter::SetAsSlave() { mMasterMode = FALSE; }
void MR_MainCharacter::SetHoverId(int pId) { mHoverId = pId; }
void MR_MainCharacter::SetHoverModel(int pModel) { mHoverModel = pModel; }
int  MR_MainCharacter::GetHoverModel()const { return mHoverModel; }
void MR_MainCharacter::SetOrientation(MR_Angle pOrientation) {
	mOrientation      = pOrientation;
	mCabinOrientation = pOrientation;
}

int MR_MainCharacter::GetHoverId()const { return mHoverId; }
...
void MR_MainCharacter::RegisterFactory() { MR_DllObjectFactory::RegisterLocalDll(MR_MAIN_CHARACTER_DLL_ID, FactoryFunc); }
...
void MR_MainCharacter::SetNbLapForRace(int pNbLap) { mNbLapForRace = pNbLap; }
...
void MR_MainCharacter::SetControlState(int pState, MR_SimulationTime pTime) {
	...
	if((mControlState & eLookBack) && !(lState & eLookBack)) // We're no longer looking behind us
...
int MR_MainCharacter::InternalSimulate(MR_SimulationTime pDuration, MR_Level *pLevel, int pRoom) {
	...
	if(mOutOfControlDuration <= 0) { // No friction when out of control..it's just more cool
		if(lAbsoluteSpeed <= -pDuration * eFrictionAccell[mHoverModel]) {
			...
			else if((lAbsoluteSpeed > eSteadySpeed[mHoverModel]) && 
					(lAbsoluteSpeed < 2.5 * eSteadySpeed[mHoverModel]))
	...
	// MotorEffect
	if(mControlState & eMotorOn) {
		double lDirectionalSpeed = (mXSpeed * MR_Cos[mCabinOrientation] + 
											mYSpeed * MR_Sin[mCabinOrientation]) / MR_TRIGO_FRACT;
		...
		if(lDirectionalSpeed < (lMaxSpeedFactor * eSteadySpeed[mHoverModel])) {
			...
			mXSpeed += (pDuration * lAccelerationFactor * eMotorAccell[mHoverModel] * 
							MR_Cos[mCabinOrientation]) / MR_TRIGO_FRACT;
			mYSpeed += (pDuration * lAccelerationFactor * eMotorAccell[mHoverModel] * 
							MR_Sin[mCabinOrientation]) / MR_TRIGO_FRACT;
		}
		...
		if(lReport.IsInMaze()) {
			if(!lReport.HaveContact()) {
				...
				if((lReport.SpaceToCeiling() > 0) &&
					(lReport.StepHeight() <= 1 - lTranslation.mZ)) {
...
void MR_MainCharacter::ApplyEffect(const MR_ContactEffect *pEffect, 
...
MR_Int32 MR_MainCharacter::Cylinder::ZMin()const { return mPosition.mZ; }
MR_Int32 MR_MainCharacter::Cylinder::ZMax()const { return mPosition.mZ + eCharacterHeight; }
MR_Int32 MR_MainCharacter::Cylinder::AxisX()const { return mPosition.mX; }
MR_Int32 MR_MainCharacter::Cylinder::AxisY()const { return mPosition.mY; }
MR_Int32 MR_MainCharacter::Cylinder::RayLen()const { return mRay; }
// Done with the cylinder stuff

MR_Angle MR_MainCharacter::GetCabinOrientation()const { return mCabinOrientation; }
double MR_MainCharacter::GetFuelLevel()const { return mFuelLevel / eFuelCapacity; }
MR_MainCharacter::eWeapon MR_MainCharacter::GetCurrentWeapon()const { return mCurrentWeapon; }
...
int MR_MainCharacter::GetMineCount()const { return mMineList.Used(); }
int MR_MainCharacter::GetPowerUpCount()const { return mPowerUpList.Used(); }
...
double MR_MainCharacter::GetDirectionalSpeed()const {
	double lReturnValue = (mXSpeed * MR_Cos[mCabinOrientation] + 
								mYSpeed * MR_Sin[mCabinOrientation]) / 
								(MR_TRIGO_FRACT * eSteadySpeed[0] * 1.9);
...
int MR_MainCharacter::GetLap()const { return mLapCount; }
int MR_MainCharacter::GetTotalLap()const { return mNbLapForRace; }
MR_SimulationTime MR_MainCharacter::GetTotalTime()const { return mLastLapCompletion; }
MR_SimulationTime MR_MainCharacter::GetBestLapDuration()const { return mBestLapDuration; }
MR_SimulationTime MR_MainCharacter::GetLastLapDuration()const { return mLastLapDuration; }
MR_SimulationTime MR_MainCharacter::GetLastLapCompletion()const { return mLastLapCompletion; }
BOOL MR_MainCharacter::HasFinish()const { return (mLapCount >= mNbLapForRace); }
int MR_MainCharacter::HitQueueCount()const { return mLastHits.Used(); }
