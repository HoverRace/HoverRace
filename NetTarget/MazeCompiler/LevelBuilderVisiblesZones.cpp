...
void MR_LevelBuilder::TestForVisibility(  MR_VisibleStep     *pPreviousStep,
                                          int                *pDestArray,
                                          int                &pDestIndex,
                                          int                 pNewLeftNodeIndex )
{
   ...
   if( lCurrentStep.mZone != -1 )
   {
      ...
      {
         lAngle = GetAngle( pPreviousStep->mLeftLimit, lCurrentStep.mRightNode,
                            GetAngle( pPreviousStep->mLeftLimitLightSource, pPreviousStep->mLeftLimit ) );
      }
      ...
      if( lAngle < -0.0001 )
      {
         ...
         {
            lAngle = GetAngle( pPreviousStep->mRightLimit, lCurrentStep.mLeftNode,
                               GetAngle( pPreviousStep->mRightLimitLightSource, pPreviousStep->mRightLimit ) );
         }
         ...
         if( lAngle > 0.0001 )
         {
            ...
            {
               lAngle = GetAngle( pPreviousStep->mLeftLimit, lCurrentStep.mLeftNode,
                                  GetAngle( pPreviousStep->mLeftLimitLightSource, pPreviousStep->mLeftLimit ) );
            }
            ...
            else
            {
               ...
               while( lStep != NULL )
               {
                  lAngle = GetAngle( lStep->mRightNode, lCurrentStep.mLeftLimit,
                                     GetAngle( lCurrentStep.mLeftLimitLightSource,
                                               lCurrentStep.mLeftLimit ));
                  if( lAngle < 0.0 )
                  {
            ...
            {
               lAngle = GetAngle( pPreviousStep->mRightLimit, lCurrentStep.mRightNode,
                                 GetAngle( pPreviousStep->mRightLimitLightSource, pPreviousStep->mRightLimit ) );
            }
            ...
            else
            {
               ...
               while( lStep != NULL )
               {
                  lAngle = GetAngle( lStep->mLeftNode, lCurrentStep.mRightLimit,
                                     GetAngle( lCurrentStep.mRightLimitLightSource,
                                               lCurrentStep.mRightLimit ));
                  if( lAngle > 0.0 )
                  {
            ...
            for( int lCounter = 0; lCounter<mRoomList[lCurrentStep.mZone].mNbVertex; lCounter++ )
            {
               ...
               if( lPos != lCurrentStep.mRightNode )
               {
                  ...
                  TestForVisibility( &lCurrentStep,
                                      pDestArray,
                                      pDestIndex,
                                      lCounter     );
               }
