		...
		case WM_INITDIALOG:
			...
			*/
			...
			if (!gsSortedTrackList.empty()) {
				...
				SetDlgItemText(hwnd, IDC_DESCRIPTION, gsSortedTrackList[gsSelectedEntry]->description.c_str());
			...
			switch (LOWORD(wparam)) {
				case IDC_LIST:
					switch (HIWORD(wparam)) {
						case LBN_SELCHANGE:
							/*
							gsSelectedEntry = SendDlgItemMessage(hwnd, IDC_LIST, LB_GETCURSEL, 0, 0);
				...
				case IDOK:
					...
					if(gsSelectedEntry != -1) {
						gsNbLaps = GetDlgItemInt(hwnd, IDC_NB_LAP, NULL, FALSE);
						gsGameOpts = 0; // reset in case something odd happened... shouldn't be necessary
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_WEAPONS_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_WEAPONS : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_MINES_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_MINES : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_CANS_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_CANS : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_BASIC_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_BASIC : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_BI_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_BI : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_CX_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_CX : 0;
						gsGameOpts |= (SendDlgItemMessage(hwnd, IDC_EON_CHK, BM_GETCHECK, 0, 0) == BST_CHECKED) ? OPT_ALLOW_EON : 0;
