...
/* Main Room */
#define IR_SEM_KEY         72327
#define IR_SHMEM_KEY       72327 
...
typedef struct SChatMessage ChatMessage;
...
typedef struct SIRState IRState;
...
BOOL ValidUserId( IRState* pThis , int pUserIndex, int pUserId );
BOOL ValidGameId( IRState* pThis , int pGameIndex, int pGameId );
...
void PrintState( IRState* pThis )
{
   ...
   for( lCounter = 0; lCounter < IR_MAX_GAME; lCounter++ )
   {
      if( pThis->mGame[ lCounter ].mUsed )
      {
         ...
         printf( "%u %d %d %d\n", pThis->mGame[ lCounter ].mPort, pThis->mGame[ lCounter ].mNbLap, pThis->mGame[ lCounter ].mWeapon, pThis->mGame[ lCounter ].mNbPlayer );

         for( lPlayer = 0; lPlayer < pThis->mGame[ lCounter ].mNbPlayer; lPlayer++ )
         {
            printf( "%d ", pThis->mGame[ lCounter ].mPlayers[ lPlayer ] );
         }
         printf( "\n" );
      }
   }
...
void PrintStateChange( IRState* pThis, int pPlayerIndex, int pPlayerId, int pTimeStamp  )
{
   ...
   else
   {
      ...
      for( lCounter = 0; lCounter < IR_MAX_CHAT_MESSAGE; lCounter++ )
      {
         lIndex = (lCounter+pThis->mChatTail)%IR_MAX_CHAT_MESSAGE;

         if( (pThis->mChatFifo[lIndex].mTimeStamp != 0)&&(pThis->mChatFifo[lIndex].mTimeStamp >= pTimeStamp) )
         {
            printf( "CHAT\n%s\n", pThis->mChatFifo[ lIndex ].mData );
         }
      }
   }
}
...
void AddGame( IRState* pThis, const char* pGameName, const char* pTrackName, int pNbLap, int pPlayerIndex, int pPlayerId, const char* pRemoteAddr, unsigned int pPort, int pWeapon )
{
   ...
   else
   {
      ...
      {
         pThis->mGame[ lCounter ].mTimeStamp  = pThis->mTimeStamp;
...
void LeaveGame( IRState* pThis, int pGameIndex, int pGameId, int pPlayerIndex, int pPlayerId  )
{
   ...
   else
   {
      ...
      /* Find player index */
      for( lCounter = 1; lCounter < pThis->mGame[ pGameIndex ].mNbPlayer; lCounter++ )
      {
      ...
      else
      {
         ...
         lNbLeft = lCounter-pThis->mGame[ pGameIndex ].mNbPlayer;

         if( lNbLeft > 0 )
         {
            memmove( &(pThis->mGame[ pGameIndex ].mPlayers[lCounter]), &(pThis->mGame[ pGameIndex ].mPlayers[lCounter+1]), sizeof( (pThis->mGame[ pGameIndex ].mPlayers[lCounter]) )*lNbLeft );
         }
