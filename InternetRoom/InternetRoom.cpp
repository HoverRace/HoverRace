					...
					if(lOK) {
						...
						for(int lCounter = IR_MAX_CLIENT - 1; lCounter >= 0; lCounter--) {
							if((mUser[lCounter].mTimeStamp != 0) && (mUser[lCounter].mExpiration != 0)) {
								if(!strcmp(mUser[lCounter].mIP, pMessage + 6) || !strcmp(mUser[lCounter].mName, pMessage + 6)) {
									// Kick and quit
									if(InitLogFile()) {
										fprintf(gLogFile, "KICK %s(%d-%d) by  %s(%d-%d) %s\n", mUser[lCounter].mName, mUser[lCounter].mKeyID.mMajor, mUser[lCounter].mKeyID.mMinor, mUser[pUserIndex].mName, mUser[pUserIndex].mKeyID.mMajor, mUser[pUserIndex].mKeyID.mMinor, mUser[lCounter].mIP);
									}
									RemoveUser(lCounter, TRUE);
									break;
								}
