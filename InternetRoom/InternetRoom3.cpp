...
class IRState
{
   ...
   public:
      ...
      void PrintStateChange( int pPlayerIndex, int pPlayerId, int pTimeStamp  ); // Refresh
      ...
      for( lCounter = 0; lCounter < IR_MAX_CHAT_MESSAGE; lCounter++ )
      {
             ...
             &&(mChatFifo[lIndex].mTimeStamp != 0))
         {
            if( (mChatFifo[lIndex].mUserDest == -1)||
                (mChatFifo[lIndex].mUserDest == pPlayerId )||
                (mChatFifo[lIndex].mUserSrc == pPlayerId )    )
            {
               Print( "CHAT\n%s\n", mChatFifo[ lIndex ].mData );
            }
         }
      ...
      if( (mUser[lCounter].mTimeStamp != 0) )
      {
         if( mUser[ lCounter ].mExpiration != 0 )
         {
            ...
            else
            {
               Print( "%s(%d-%d)<br>\n", 
                      mUser[ lCounter ].mName,
                      mUser[ lCounter ].mKeyID.mMajor,
                      mUser[ lCounter ].mKeyID.mMinor  );
            }
         }
      }
   }
            ...
            if( lKick )
            {
               ...
               if( lOK )
               {
                  ...
                  for( int lCounter = IR_MAX_CLIENT-1; !lUserFound && (lCounter >= 0); lCounter-- )
                  {
                     if( (mUser[lCounter].mTimeStamp != 0)&&(mUser[ lCounter ].mExpiration != 0) )
                     {
                        if( !strcmp( mUser[ lCounter ].mIP, lUser )  ||
                            !strcmp( mUser[ lCounter ].mName, lUser )||
                            ((mUser[ lCounter ].mKeyID.mMajor == lToBanMajor)&&
                             (mUser[ lCounter ].mKeyID.mMinor == lToBanMinor))
                          )
                        {
                           lUserFound = TRUE;
                           ...
                           if( InitLogFile() )
                           {
                              fprintf( gLogFile, "KICK %s(%d-%d) by  %s(%d-%d) for %d min %s\n",
                                       ...
                                       mUser[ lCounter ].mKeyID.mMajor,
                                       mUser[ lCounter ].mKeyID.mMinor,
                                       mUser[ pUserIndex ].mName,
                                       mUser[ pUserIndex ].mKeyID.mMajor,
                                       mUser[ pUserIndex ].mKeyID.mMinor,
                                       lBanDuration,
                                       mUser[ lCounter ].mIP
                                     );
                           }
                           ...
                           {
                              sprintf( lBuffer, 
                                       "%s has been kicked by %s(%d-%d) for %d min",
                                       mUser[ lCounter ].mName,
                                       mUser[ pUserIndex ].mName,
                                       mUser[ pUserIndex ].mKeyID.mMajor,
                                       mUser[ pUserIndex ].mKeyID.mMinor,
...
//                     lDoubleLog = TRUE;
         ...
         if( !lNotAllowed && !lTempBanned && !lBanned && !lExpired && !lDoubleLog && !lNotReg && !lImposter && !lNotVerified)
         {
            ...
            if( pMajorID != -1 )
            {
               ...
               {
                  if( mUser[ lCounter ].mTrusted 
                      &&( gSUList[ lSUIndex ].mMajor == pMajorID )
...
class IR_Port
{
   public:
      ...
      char   mBuffer[ PORT_BUFFER_SIZE ];
      int    mBufferIndex;
      char   mIP[16];
      time_t mConnectionTime;
...
void IR_Port::Accept( SOCKET pParent )
{
   ...
   mPort = accept( pParent, (struct sockaddr*)&lAddr, &lAddrSize );

   if( mPort != INVALID_SOCKET )
   {
      unsigned lIP = ntohl( lAddr.sin_addr.s_addr );
      ...
      int lValue = 8192;
      setsockopt( mPort, SOL_SOCKET, SO_SNDBUF, (const char*)&lValue, sizeof( lValue ) );
      
      mConnectionTime = time( NULL );
   }
}
...
void IR_Port::Read()
{
   if( mPort != INVALID_SOCKET )
   {
      int lToRead =  sizeof( mBuffer )-1-mBufferIndex;
      ...
      {
         int lRead = recv( mPort, mBuffer+mBufferIndex, lToRead, 0 );
...
IR_Port* gCurrentPort = NULL;

SOCKET  lMasterSocket;
IR_Port lConnection[ MAX_CONNECT ];
...
void AcceptSockets()
{
   ...
   time_t lExpiredTime = time( NULL )-30;
   ...
   while( 1 )
   {
      ...
      for( ; lSocket<MAX_CONNECT; lSocket++ )
      {
         ...
         else if( lConnection[ lSocket ].mConnectionTime < lExpiredTime )
         {
      ...
      {
         lConnection[ lSocket ].Accept( lMasterSocket );            
...
int main( int pArgc, const char** pArgs )
{
   ...
   if( (pArgc == 3)||(pArgc == 4) )
   {
      sscanf( pArgs[1], "%d", &lMasterPort );
      sscanf( pArgs[2], "%s",  gLogFileName );
   ...
   lMasterSocket = socket( PF_INET, SOCK_STREAM, 0 );

   SOCKADDR_IN lAddr;

   lAddr.sin_family      = AF_INET;
   lAddr.sin_addr.s_addr = INADDR_ANY;
   lAddr.sin_port        = htons( lMasterPort );

   if( bind( lMasterSocket, (SOCKADDR*)&lAddr, sizeof(lAddr)) != 0 )
   { 
                  ...
                  else
                  {
                     ...
                     if( lState != NULL )
                     {
                        ...
                        else if( !strcmp( lOp, "ADD_GAME" ) )
                        {
                           ...
                           {
                              const char* lRemoteAddr = GetRemoteAddr();
                        ...
                        else if( !strcmp( lOp, "WWW_MADD" ) )
                        {
                           ...
                           if( sscanf( lQuery, "%*s width=%d mes=%200s", &lWidth, lMessage )>=1 )
                           {
                              WWWUnpad( lMessage );
                              ChatFilter( lMessage );
                           ...
                           Print( "<input type=\"text\" name=\"mes\" value=\"\" size=%d  maxlength=120>\n", lWidth );
...
static void Print( const char* pFormat, ... )
{
   ...
   lLen = vsprintf( lBuffer, pFormat, lArgPtr );
   va_end( lArgPtr );
